"""Отрисовка платёжного QR-кода в формате PNG.

Модуль решает две задачи сверх обычного вызова segno.

Логотип в центре перекрывает часть модулей кода, поэтому при его наличии
повышается уровень коррекции ошибок, а размер логотипа ограничивается
долей от ширины изображения (:data:`LOGO_SIZE_RATIO`).

Цветом задаются только модули; фон остаётся белым. Прозрачный фон на
тёмной подложке делает код нечитаемым для сканера.
"""
