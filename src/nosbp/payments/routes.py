...
async def _generate(
    *,
    ...
    # Картинка одного и того же счёта не меняется, поэтому клиенту незачем
    # скачивать её повторно. Это снимает с сервиса основную часть нагрузки:
    # почтовые прокси кэшируют изображение у себя.
