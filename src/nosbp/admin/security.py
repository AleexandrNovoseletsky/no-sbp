...
Панель распоряжается чужими деньгами, поэтому вход устроен строже, чем
обычно бывает у внутренних инструментов:
...
* сессия живёт в базе, а не в подписанной куке: её можно отозвать
  мгновенно, и перезапуск сервиса не выкидывает администратора наружу;
...
# Параметры argon2id. Значения по умолчанию из библиотеки подобраны
# разумно; фиксируем их явно, чтобы обновление зависимости не поменяло
# стойкость молча.
...
def verify_password(password_hash: str, password: str) -> bool:
    """Проверяет пароль. Никогда не бросает исключений наружу.

    UnicodeError ловится наравне с ошибками argon2: библиотека кодирует
    строку хэша в ASCII, и испорченная запись в базе иначе уронила бы
    страницу входа вместо честного «неверный пароль».
    """
    try:
