"""Тесты одноразовых ссылок для установки пароля.

Закрывают случай, при котором аккаунт, заведённый оператором, оставался
недоступным: пароля у него нет, а зарегистрироваться на занятый адрес
нельзя.
"""
...
from nosbp.db.models import AccountInvite
...
async def test_issue_returns_token_and_stores_only_hash(session, make_account):
    ...
    token = await invites.issue(session, account, ttl_hours=72)
...
async def test_new_link_cancels_the_previous_one(session, make_account):
    ...
    account = await make_account()
    first = await invites.issue(session, account, ttl_hours=72)
    await session.commit()

    await invites.issue(session, account, ttl_hours=72)
    await session.commit()

    with pytest.raises(ValidationError, match="недействительна"):
        await invites.find_account(session, first)
...
async def test_expired_link_is_refused(session, make_account):
    ...
    with pytest.raises(ValidationError, match="недействительна"):
        await invites.find_account(session, token)


async def test_unknown_link_is_refused(session):
    with pytest.raises(ValidationError, match="недействительна"):
        await invites.find_account(session, "выдуманный токен")
...
def test_url_is_built_from_settings():
    url = invites.build_url("https://example.com/", "/cabinet", "TOKEN")
...
async def test_forgot_page_points_to_support(cabinet):
    response = await cabinet.get(f"{CABINET_PREFIX}/forgot")
    assert response.status_code == 200
    assert "Не получается войти" in response.text


async def test_mismatched_passwords_on_link(cabinet, session, make_account):
    account = await make_account()
    token = await invites.issue(
        session, account, ttl_hours=get_settings().cabinet_invite_ttl_hours
    )
    await session.commit()
