...
- [Упрощённый сервис](#упрощённый-сервис)
...
В каталоге [`solo/`](solo/) лежит урезанная сборка на случай, когда
полный сервис поднять негде: например, основной сервер недоступен,
а QR-коды нужны сегодня.

Это один контейнер без базы данных и объектного хранилища: одна
организация, реквизиты из `.env`, только `GET /v1/qr`. Разворачивается
на любом сервере, где есть Docker, — в том числе рядом с сайтом
компании.

Платёжное ядро у него общее с полным сервисом: те же модули строки ГОСТ
и отрисовки кода, те же имена параметров и заголовки ответа. При
переключении в шаблонах CRM меняется только адрес, а картинка остаётся
побайтово той же — это проверяет `tests/test_solo.py`.

Инструкция по развёртыванию — [`solo/README.md`](solo/README.md).
...
Код копируется в образ, а не подключается томом, поэтому `build`
обязателен: без него `up -d` перезапустит контейнер с прежней версией
сервиса. Настройки же читаются из `.env` при каждом старте — после их
правки достаточно `up -d`.
...
Здесь приложение запускается напрямую и подхватывает правки на лету.
Сервис из `docker compose up -d api` (порт 8080) собран в образ и правок
в исходниках не видит — после них нужен `docker compose build api`.
...
solo/          упрощённая сборка: один контейнер, одна организация, только GET
