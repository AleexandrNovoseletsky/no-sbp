...
заказчик не сможет. Доступ выдаётся одноразовой ссылкой: на карточке
заказчика в панели есть кнопка «Создать ссылку для установки пароля».
Ссылка действует `CABINET_INVITE_TTL_HOURS` часов, гасится после
использования и отменяет ранее выданные.

Тем же способом восстанавливается забытый пароль: писем сервис пока
не отправляет, поэтому заказчик обращается в поддержку, а оператор
выдаёт ссылку. Адрес поддержки задаётся переменной `SUPPORT_EMAIL`
и показывается на странице «не получается войти».
